...
CREATE TABLE scores (
  ...
  total_score INTEGER GENERATED ALWAYS AS (
    CASE 
      ...
      WHEN session_type = 'judging' THEN (innovation + creativity + feasibility + presentation + COALESCE(design, 0) + COALESCE(user_experience, 0)) * 10 / 6 * 5
      ELSE 0
    END
  ) STORED,
  final_score NUMERIC GENERATED ALWAYS AS (
    CASE 
      WHEN session_type = 'mentoring' THEN ROUND((innovation + creativity + feasibility + presentation)::numeric * 10, 2)
      WHEN session_type = 'judging' THEN ROUND((innovation + creativity + feasibility + presentation + COALESCE(design, 0) + COALESCE(user_experience, 0))::numeric * 10 / 6 * 5, 2)
      ELSE 0
    END
  ) STORED,
  comments TEXT,
